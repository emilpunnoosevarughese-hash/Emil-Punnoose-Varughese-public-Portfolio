...
export const SourceAttribution: React.FC<SourceAttributionProps> = ({ 
  ...
  return (
    <div className={`space-y-4 ${compact ? 'text-sm' : 'text-base'}`}>
      {source && (
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 justify-between p-3 rounded-lg border border-gray-200 dark:border-gray-800 glass-effect">
          <div className="flex items-center gap-2">
            <ShieldCheck className="text-green-500" size={18} />
            <span className="font-medium text-gray-900 dark:text-gray-100">Sources & Verification</span>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            {source.url && (
              <a 
                ...
                className="flex items-center gap-1 text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300 transition-colors text-xs"
              >
      ...
      {disclaimer && (
        <div className="flex items-start gap-2 p-3 rounded-lg bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 text-xs text-gray-500 dark:text-gray-400">
          <Info size={16} className="shrink-0 mt-0.5 text-gray-400 dark:text-gray-500" />
          <p>
